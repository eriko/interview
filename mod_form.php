...
class mod_interview_mod_form extends moodleform_mod
{
	...
	function definition()
	{
		...
		global $CFG;
		$mform =& $this->_form;
		if (isset($this->current->timeopen)) {
			echo "isset timeopen";
			echo  $this->current->timeopen;
		}

		$this->enableavailability = true;
		...
		if (!isset($this->current->timeopen)) {
			$mform->setDefault('timeopen', time() + 1 * 24 * 3600);
		} else {
			$mform->setDefault('timeopen', $this->current->timeopen);
		}
		...
		if (!isset($this->current->timeclose)) {
			$mform->setDefault('timeclose', time() + 2 * 24 * 3600);
		} else {
			$mform->setDefault('timeclose', $this->current->timeopen);
		}
	...
	function validation($data)
	{
		...
		} elseif (!isset($this->current->timeopen) && $data['timeopen'] < time()) {
		...
		} elseif (!isset($this->current->timeclose) && $data['timeclose'] <= time()) {
